...
public class PythonTruthTableServer {
    ...
    public static Vector<Vector<Vector<String>>> dnfParse(String expr) {
        ...
        if (response.toString().contains("Or") && response.toString().contains("And")) {
            ...
            Vector<String> dnfVec = arrToVec(dnf.split("(\\), And\\()|(\\),)|(, And\\()|(And\\()"));
            dnfVec.removeIf(String::isEmpty);
            dnfVec.forEach(s -> {
                if (s.contains(", ")) {
                    String replace = s.trim().replace(", ", "%26");
                    lineTable.add(truthTableParse(logicParse(replace), replace));
                } else {
                    lineTable.add(truthTableParse(s, s));
                }
            });
    ...
    public static Vector<Vector<Vector<String>>> cnfParse(String expr) {
        StringBuilder response = new StringBuilder();
        try {
            // Define the server URL with the parameter
            String serverUrl = "http://localhost:8000/cnf?param=" + expr;
        ...
        Vector<Vector<Vector<String>>> lineTable = new Vector<>();
        if (response.toString().contains("Or") && response.toString().contains("And")) {
            String cnf = response.substring(4, response.length() - 2);
            Vector<String> cnfVec = arrToVec(cnf.split("(\\), Or\\()|(\\),)|(, Or\\()|(Or\\()"));
            cnfVec.removeIf(String::isEmpty);
            cnfVec.forEach(s -> {
                if (s.contains(", ")) {
                    String replace = s.trim().replace(", ", "%26");
                    lineTable.add(truthTableParse(logicParse(replace), replace));
            ...
            String and = response.substring(3, response.length() - 1);
            and = and.replace(", ", "%26");
            lineTable.add(truthTableParse(logicParse(and), and));
        } else if (response.toString().contains("And")) {
            String dnf = response.substring(4, response.length() - 1);
            Vector<String> dnfVec = arrToVec(dnf.split(", "));
            dnfVec.forEach(s -> lineTable.add(truthTableParse(logicParse(s), s)));
        } else {
            lineTable.add(truthTableParse(response.toString(), response.toString()));
