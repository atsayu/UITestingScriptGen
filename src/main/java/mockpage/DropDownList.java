...
public class DropDownList extends ProcessDetectElement {
  ...
  private void storeMapTextAndChoiceToSelectAndOptionElement(Element root,String text, Element selectElement) {
    ...
    if (tagNameCurrentElement.equals("option") && mapStoreNormalizeTextAndChoices.containsKey(text) && mapStoreNormalizeTextAndChoices.get(text).contains(normalizeTextInCurrentElement)) {
